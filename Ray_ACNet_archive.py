...
class ACNet:
    def __init__(self, scope, a_size, trainer, TRAINING, NUM_CHANNEL, OBS_SIZE, GLOBAL_NET_SCOPE, GLOBAL_NETWORK=False):
        ...
        if GLOBAL_NETWORK:
            ...
            # Additional code for clipping gradients if needed
    ...
    def _build_net(self, inputs, goal_pos, RNN_SIZE, TRAINING, a_size):
        ...
        def VGG_Block(inputs):
            def conv_2d(inputs, kernal_size, output_size):
                ...
                return conv 
            # def conv_2d(inputs, kernel_size, output_size, w_init=None):
            #     conv = tf.keras.layers.Conv2D(filters=output_size,
            #                                 kernel_size=kernel_size,
            #                                 strides=(1, 1),
            #                                 padding='SAME',
            #                                 data_format='channels_last',  # Equivalent to NHWC
            #                                 kernel_initializer=w_init,    # Adjust initializer as needed
            ...
            # updated to keras
            # pool1 = tf.keras.layers.MaxPooling2D(pool_size=[2, 2], strides=2, padding='VALID')(conv1b)
            return pool1
        ...
        #     filters=RNN_SIZE - GOAL_REPR_SIZE,  # num_outputs in TensorFlow 1.x
        #     kernel_size=[2, 2],
        #     strides=(1, 1),  # Stride
        #     padding='VALID',  # Padding
        #     data_format='channels_last',  # NHWC
        #     kernel_initializer=w_init,  # Weights initializer
        #     activation=None  # activation_fn
        # )(vgg2)

        flat = tf.nn.relu(layers.flatten(conv3))
        # updated to keras and TF2:
        # flat = tf.keras.layers.Flatten()(conv3)
        # flat = tf.keras.activations.relu(flat)
        ...
        # # New Implementation
        # # Fully connected layer for goal_pos
        # goal_layer = tf.keras.layers.Dense(GOAL_REPR_SIZE, activation='relu')(goal_pos)
        # # Concatenation of flat and goal_layer
        # hidden_input = tf.keras.layers.Concatenate(axis=1)([flat, goal_layer])
        # # First fully connected layer
        # h1 = tf.keras.layers.Dense(RNN_SIZE, activation='relu')(hidden_input)
        # # Dropout layer
        # d1 = tf.keras.layers.Dropout(rate=1-KEEP_PROB1)(h1, training=TRAINING)
        # # Second fully connected layer without activation function (None)
        # h2 = tf.keras.layers.Dense(RNN_SIZE)(d1)
        # # Second dropout layer
        # d2 = tf.keras.layers.Dropout(rate=1-KEEP_PROB2)(h2, training=TRAINING)
        # # Adding the output of the second dropout layer to hidden_input and applying ReLU
        # self.h3 = tf.keras.activations.relu(d2 + hidden_input)
        ...
        # New Implementation for Tensorflow 2.x:
        # Policy layer without activation function 
        ...
        # # Applying softmax and sigmoid activations
        # policy = tf.nn.softmax(policy_layer)
        # policy_sig = tf.sigmoid(policy_layer)
